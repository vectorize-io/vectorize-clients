...
class Turbopuffer(BaseModel):
    ...
    type: StrictStr = Field(description="Connector type (must be \"TURBOPUFFER\")")
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TURBOPUFFER']):
            raise ValueError("must be one of enum values ('TURBOPUFFER')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "type": obj.get("type"),
            "config": TURBOPUFFERAuthConfig.from_dict(obj["config"]) if obj.get("config") is not None else None
        })
        return _obj
