...
class FileUpload(BaseModel):
    ...
    type: StrictStr = Field(description="Connector type (must be \"FILE_UPLOAD\")")
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FILE_UPLOAD']):
            raise ValueError("must be one of enum values ('FILE_UPLOAD')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "type": obj.get("type"),
            "config": FILEUPLOADAuthConfig.from_dict(obj["config"]) if obj.get("config") is not None else None
        })
        return _obj
