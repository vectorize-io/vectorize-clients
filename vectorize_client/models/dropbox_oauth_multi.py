...
class DropboxOauthMulti(BaseModel):
    """
    DropboxOauthMulti
    """ # noqa: E501
    name: StrictStr = Field(description="Name of the connector")
    type: StrictStr = Field(description="Connector type (must be \"DROPBOX_OAUTH_MULTI\")")
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DROPBOX_OAUTH_MULTI']):
            raise ValueError("must be one of enum values ('DROPBOX_OAUTH_MULTI')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "type": obj.get("type"),
            "config": DROPBOXOAUTHMULTIAuthConfig.from_dict(obj["config"]) if obj.get("config") is not None else None
        })
        return _obj
