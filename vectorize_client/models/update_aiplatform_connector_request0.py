...
class UpdateAiplatformConnectorRequest(BaseModel):
    ...
    """
    # data type: Bedrock1
    oneof_schema_1_validator: Optional[Bedrock1] = None
    # data type: Vertex1
    oneof_schema_2_validator: Optional[Vertex1] = None
    # data type: Openai1
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = UpdateAiplatformConnectorRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: Bedrock1
        if not isinstance(v, Bedrock1):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Bedrock1`")
        else:
            match += 1
        # validate data type: Vertex1
        if not isinstance(v, Vertex1):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Vertex1`")
        else:
            match += 1
        # validate data type: Openai1
        if not isinstance(v, Openai1):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Openai1`")
        else:
            match += 1
        # validate data type: Voyage1
        if not isinstance(v, Voyage1):
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into Bedrock1
        try:
            instance.actual_instance = Bedrock1.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into Vertex1
        try:
            instance.actual_instance = Vertex1.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into Openai1
        try:
            instance.actual_instance = Openai1.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into Voyage1
        try:
            instance.actual_instance = Voyage1.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
